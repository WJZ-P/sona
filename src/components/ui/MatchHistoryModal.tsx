...
function formatDate(timestamp: number): string {
  ...
  if (diffDays === 2) return `${translate('common.beforeYesterday')} ${time}`
...
function MatchRow({ match, onOpenDetail }: { match: MatchRowData; onOpenDetail: (gameId: number) => void }) {
  ...
  const { t } = useI18n()
  const statusText = match.win ? t('common.win') : t('common.loss')
  ...
  return (
    <div
      ...
      title={t('matchHistory.clickDetail')}
    >
...
export function MatchHistoryModal({ open, onClose, puuid, playerName, queueId: defaultQueueId }: MatchHistoryModalProps) {
  ...
  const loadMatches = useCallback(async (queueId: number) => {
    ...
    try {
      ...
      setError(t('matchHistory.loadFailed'))
    } finally {
      setLoading(false)
    }
  }, [puuid])
    ...
    <Modal open={open} onClose={onClose} width={860} height={620}>
      <div className="smh-container">
        ...
        <div className="smh-list" ref={listRef}>
          {loading && <div className="smh-empty">{t('common.loading')}</div>}
          {error && <div className="smh-empty smh-error">{error}</div>}
          {!loading && !error && matches.length === 0 && (
            <div className="smh-empty">{filterQueueId > 0 ? t('matchHistory.emptyForMode') : t('matchHistory.empty')}</div>
          )}
          {matches.map((m) => (
          ...
          {!loading && !error && matches.length > 0 && (
            <div className="smh-empty smh-no-more">
              {hasMore ? t('matchHistory.scrollMore') : t('matchHistory.total', { count: matches.length })}
            </div>
          )}
        </div>
        <MatchDetailModal
