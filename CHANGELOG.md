...
- **OP.GG 配装推荐增强** — 新增选人阶段配装、符文、召唤师技能与强化符文推荐，并优化大乱斗、斗魂与海克斯推荐展示。
- **英雄选择阶段增强** — 增加英雄 T 级角标、备选席胜率、头像预选/锁定 T 级提示，并将相关显示统一跟随「英雄选择阶段增强」开关。
- **更新提醒** — 启动时检测新版本，面板展示更新内容，并在入口按钮和客户端通知中提示。
- **客户端请求拦截** — 新增 XHR 拦截体系，用于阻断比赛直播弹窗等不需要的客户端请求。

### 🐛 Bug 修复

- **选人阶段数据错位** — 修复玩家换位后胜率/KDA 与头像点击战绩仍绑定旧楼层的问题。
- **开黑好友标记首次不显示** — 改为监听好友状态 WebSocket 更新并刷新右侧好友标记。
- **ARAM.GG 数据解析** — 支持从 Next.js HTML 响应中提取 RSC 数据，提升推荐解析稳定性。
